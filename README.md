...
### K 的影响：论文的 K=50 我也实测了

上表里"论文基准（K=50）"那一行，我一开始是照抄的。后来把 15 类在 `K=50` 下完整跑了一遍（`artifacts/runs/full-mvtec-k50/`），结果分成两半：

| | 图像级 ROC-AUC | 像素级 ROC-AUC | 耗时 |
| --- | ---: | ---: | ---: |
| 论文自报（K=50） | 85.5 | 96.5 | — |
| 我的 K=5 | **85.41** | 96.44 | 19.4 min |
| 我的 K=50 | 80.88 | **96.98** | 91.1 min |

- **像素级：K=50 更好**，96.98，比论文自报的 96.5 还高 0.48——定位受益于大 10 倍的 gallery。
- **图像级：K=50 反而掉 4.53 点**，离论文的 85.5 更远。论文在 K=50 下报告的 85.5，在这套实现里只有 K=5 才复现得出来。**这是一处没能复现的地方，我如实记着。**

掉分的原因我试错了一次：先猜"K=50 占小训练集比例太大"，但 K/n_train 与跌幅的相关只有 −0.19 且方向相反。换成"K=5 时图像级本来就弱的类掉得更多"，相关性 **+0.785**——K=5 图像级 < 90 的 8 类平均掉 7.57 点，≥ 90 的 7 类只掉 1.05 点。K=50 不是引入新问题，是把仅有的一点信号在 50 个邻居里平均掉了。详见 [docs/method.md](docs/method.md#7-k-的影响论文的-k50-实测)。
